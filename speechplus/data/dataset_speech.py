...
class TTSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                text_understanding_start = (
                    input_ids == self.special_tokens["<|TEXT_UNDERSTANDING_START|>"]).nonzero()
                text_understanding_end = (
                    input_ids == self.special_tokens["<|TEXT_UNDERSTANDING_END|>"]).nonzero()
                speech_start_positions = (
                    input_ids == self.special_tokens["<|SPEECH_GENERATION_START|>"]).nonzero()
                speech_end_positions = (
                    input_ids == self.special_tokens["<|SPEECH_GENERATION_END|>"]).nonzero()

                # Verify all required tokens are present
                if (len(text_understanding_start) == 0 or len(text_understanding_end) == 0 or
                        len(speech_start_positions) == 0 or len(speech_end_positions) == 0):
                    self.skipped_samples += 1
                    if self.skipped_samples % 100 == 0:
                        print(
                            f"Skipped {self.skipped_samples}/{self.total_samples_seen} samples due to missing tokens."
                        )
                ...
                prompt_ids = self.replace_token(
                    prompt_ids,
                    self.special_tokens["<|TEXT_UNDERSTANDING_START|>"],
                    text_content,
                )
                prompt_ids = self.replace_token(
                    prompt_ids,
                    self.special_tokens["<|SPEECH_GENERATION_START|>"],
                    speech_content,
                )

                # Convert to tensor and pad to max length
                input_ids = self.pad_to_max_length(
                    torch.tensor(prompt_ids, dtype=torch.long), self.pad_token_id)
    ...
    def pad_to_max_length(self, sequence, pad_value):
        ...
        return torch.cat([
            sequence,
            torch.full((self.max_length - len(sequence), ), pad_value, dtype=sequence.dtype),
        ])
...
class ASRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speech_understanding_start = (
            input_ids == self.special_tokens["<|SPEECH_UNDERSTANDING_START|>"]).nonzero()
        speech_understanding_end = (
            input_ids == self.special_tokens["<|SPEECH_UNDERSTANDING_END|>"]).nonzero()
        ...
        if (len(speech_understanding_start) == 0 or len(speech_understanding_end) == 0 or
                len(text_generation_start) == 0 or len(text_generation_end) == 0):
        ...
        prompt = [
            {
                "role": "user",
                "content": "Transcribe the following speech:<|SPEECH_UNDERSTANDING_START|>",
            },
            {
                "role": "assistant",
                "content": "<|TEXT_GENERATION_START|>"
            },
        ]
        ...
        prompt_ids = self.replace_token(
            prompt_ids,
            self.special_tokens["<|SPEECH_UNDERSTANDING_START|>"],
            speech_content,
        )
        prompt_ids = self.replace_token(
            prompt_ids, self.special_tokens["<|TEXT_GENERATION_START|>"], text_content)
