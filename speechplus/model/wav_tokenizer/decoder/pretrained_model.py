...
class WavTokenizer(nn.Module):
    ...
    def from_pretrained0828(self, config_path, model_path):
        ...
        for k, v in state_dict_raw.items():
            if (k.startswith("backbone.") or k.startswith("head.") or k.startswith("feature_extractor.") or
                    k.startswith("multiperioddisc.") or k.startswith("multiresddisc.")):
                state_dict[k] = v
    ...
    def codes_to_features(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert isinstance(self.feature_extractor,
                          EncodecFeatures), ("Feature extractor should be an instance of EncodecFeatures")
        ...
        features = torch.nn.functional.embedding(embeddings_idxs,
                                                 self.feature_extractor.codebook_weights).sum(dim=0)
