...
from .model import WavDecoder, WavEncoder
...
class AudioCodec:
    ...
    def __init__(
        ...
        if model_path is None:
            ...
            if not os.path.isdir(model_path):
                raise ValueError(
                    f"Model path {model_path} is not a directory. Please provide a valid directory path.")
            self.cache_dir = model_path

        self.encoder_path = os.path.join(self.cache_dir, "encoder")
        if not os.path.isdir(self.encoder_path):
            raise ValueError(
                f"Encoder directory not found at {self.encoder_path}. The model path must contain an 'encoder' subdirectory."
            )

        self.decoder_path = os.path.join(self.cache_dir, "decoder")
        if not os.path.isdir(self.decoder_path):
            raise ValueError(
                f"Decoder directory not found at {self.decoder_path}. The model path must contain a 'decoder' subdirectory."
            )
    ...
    def get_cache_dir(self):
        return os.path.join(
            os.getenv("APPDATA") if platform.system() == "Windows" else os.path.join(
                os.path.expanduser("~"), ".cache"),
            "outeai",
            "tts",
            "wavtokenizer_75_token_interface",
        )
